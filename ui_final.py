...
fp = Path("C:\\Anil\\Hrithik\\User_Interface\\hashed_passwords.pkl")
...
if authentication_status : 
    ...
    menu = ["🏠 Home","🍞 Food","🌍 NGO Connectivity","📈 Restaurant Visitor Forecasting","🙍 Information"]
    ...
    if choice == "🏠 Home":
        ...
        image = Image.open("C:\\Anil\\Hrithik\\User_Interface\\home_pic.jpg") 
            ...
            # Convert the file to an opencv image.
                ...
                if response_emotion['output']==1:
                    ...
                    for items in data:
                        st.subheader(f"{data[i]} ")
                        i=i+1
                if response_emotion['output']==2:
                    def get_data():
                        db = client.mydb #establish connection to the 'sample_guide' db
                        items = db.result_table.find({'flavor':{'$in':['Fast foods','sweets']}},{'foodname':1,'_id':0}) # return all result from the 'planets' collection
                        items = list(items)        
                        return items
                    data = get_data()
                    i=0
                    for items in data:
                        ...
                        i=i+1 
                        db = client.mydb 
                        db.result_table.update_one({'flavor':'sweets'},{'$inc':{"quantity":-1}})
                        db.result_table.update_one({'flavor':'Fast foods'},{'$inc':{"quantity":-1}})
                ...
                db=client.mydb
                if response_gender['output']==0:
                    #db.customer.delete_many({})
                    db.customer.insert_one({"count":1})
                    print("Male count: "+ str(db.customer.count_documents({})))
                if response_gender['output']==1:
                    #db.female.delete_many({})
                    db.female.insert_one({"count":1})
                    print("Female count: "+str(db.female.count_documents({})))
                if response_age['output']==0:
                    db.age_0.insert_one({"count":1})
                if response_age['output']==1:
                    db.age_1.insert_one({"count":1})
                if response_age['output']==2:
                    db.age_2.insert_one({"count":1})

                male_customer_count = st.button("Male Customer Count")
                if male_customer_count:
                    st.subheader("Male count: "+str(db.customer.count_documents({})))
                female_customer_count = st.button("Female Customer Count")
                if female_customer_count:
                    st.subheader("Female count: "+str(db.female.count_documents({})))
                age0_customer_count = st.button("Customer count in the range 0 to 25 years")
                if age0_customer_count:
                    st.subheader("Customer count: "+str(db.age_0.count_documents({})))
                age1_customer_count = st.button("Customer count in the range 25 to 45 years")
                if age1_customer_count:
                    st.subheader("Customer count: "+str(db.age_1.count_documents({})))
                age2_customer_count = st.button("Customer count in the range 45 and above")
                if age2_customer_count:
                    st.subheader("Customer count: "+str(db.age_2.count_documents({})))
                st.text("")
    ...
    if choice=="🙍 Information":
        ...
        if ch=="Informative Plots":
            ...
            st.text("")
            df=pd.read_csv("C:\\Anil\\Hrithik\\User_Interface\\final_dataset.csv")
    ...
    if choice=="🌍 NGO Connectivity":
        place = st.text_input("Enter the address of the restaurant","Church Street Social,Church St 46/1 Cobalt Building, Bengaluru 560001 India")
        st.markdown(place)
        if place is not None:
            ...
            #m = get_user_data("https://atlas.microsoft.com/map/static/png?api-version=1.0&style={dark}&center={"+str(longitude)+","+str(latitude)+"}")
            ...
            def get_data():
                db = client.mydb #establish connection to the 'sample_guide' db
                items = db.result_table.find() # return all result from the 'planets' collection
                items = list(items)        
                return items
            list_for_message_data = get_data()
            li = []
            for item in list_for_message_data:
                li.append([item['foodname'],item['quantity']])
            #print(li)
            pywhatkit.sendwhatmsg('+91 9481634956',"Hi. We have food remaining and would like to donate it today. Please respond to initiate further communication."+str(li),now.hour,now.minute,25,True,15)
    
    if choice == "📈 Restaurant Visitor Forecasting":
        st.subheader("RESTAURANT VISITOR FORECASTING")
        image = Image.open("C:\\Anil\\Hrithik\\User_Interface\\Forecasting.jpeg") 
        st.image(image,width = 600)
        st.markdown('''The management of inventories is aided by accurate visitor predictions. Based on expected visitor numbers, restaurants can predict the need for ingredients, beverages, and other supplies. This reduces waste and maximises cost effectiveness by preventing shortages or surplus inventory. Additionally, it makes menu planning easier and guarantees that popular items are accessible during peak times.
                    Restaurants can predict the expected foot traffic by examining historical data, seasonal patterns, and variables like holidays or special events. By using this information, managers may assign the right amount of employees to each shift and avoid understaffing or overstaffing situations. Wait times are reduced, service quality increases, and staff members are better able to meet client needs.''')
    ...
    if choice=="FAQ's":
        ...
        image_faq = Image.open("C:\\Anil\\Hrithik\\User_Interface\\FAQ.jpg") 
